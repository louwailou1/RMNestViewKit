...
#import "UIView+RMRefresh.h"

const CGFloat RMRefreshHeaderHeight = 35.0;
...
@implementation RMRefreshHeader

- (instancetype)init {
    if (self = [super initWithFrame:CGRectMake(0, -RMRefreshHeaderHeight, RMRefreshHeaderHeight, RMRefreshHeaderHeight)]) {
        [self initLayers];
    }
    return self;
}
...
- (void)initLayers {
    CGFloat centerLine = RMRefreshHeaderHeight / 2;
    ...
    CGPoint bottomPoint = CGPointMake(centerLine, RMRefreshHeaderHeight - radius);
    self.BottomPointLayer = [self layerWithPoint:bottomPoint color:bottomPointColor];
    [self.layer addSublayer:self.BottomPointLayer];
    
    CGPoint rightPoint = CGPointMake(RMRefreshHeaderHeight - radius, centerLine);
...
//        self.scrollView = (UIScrollView *)newSuperview;
        self.center = CGPointMake(newSuperview.centerX, self.centerY);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"contentOffset"]) {
        ...
        self.progress = - contentOffsetY;
//        self.y = -RMRefreshHeaderHeight - contentOffsetY;
    }
}
...
- (void)setProgress:(CGFloat)progress {
    ...
    if (!self.animating) {
        if (progress >= SURefreshPullLen) {
            self.y = - (SURefreshPullLen - (SURefreshPullLen - RMRefreshHeaderHeight) / 2);
        }else {
            if (progress <= self.h) {
                self.y = - progress;
            }else {
...
- (void)startAni {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        for (RMNestBaseController *vc in self.subVCArr) {
